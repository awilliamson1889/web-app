...
class TestApiPermission(unittest.TestCase):
    ...
    wrong_json_msg = "Permission 1 validation error for PermissionSchema\nname\n  field required (type=value_error.missing)"
    not_valid_json_msg = "Permission 1 validation error for PermissionSchema\nname\n  Name length too big! (type=value_error)"

    wrong_json = {"name_field": "Some Permission1"}

    post_valid_data = {"name": "POST ADDRESS"}
    put_valid_data = {"name": "PUT ADDRESS"}
    post_invalid_data = {"name": "Some Permission1 Some Permission1 Some Permission1 Some Permission1 Some Permission1 Some Permission1"
                                 "Some Permission1 Some Permission1 Some Permission1 Some Permission1 Some Permission1 Some Permission1"
                                 "Some Permission1 Some Permission1 Some Permission1 Some Permission1 Some Permission1 Some Permission1"}
    ...
    def test_get_permission_wrong_id_case1(self, mock_abort):
        ...
        mock_abort.assert_called_once_with(404, message="No such permission with ID=one")
    ...
    @patch('department_app.rest.permission.abort')
    @patch('department_app.rest.permission.CRUDPermission.get')
    def test_get_permission_wrong_id_case3(self, mock_get_permission, mock_abort):
        """Api should return information about permission."""
        mock_get_permission.return_value = None
        Permission.get(1.0)

        mock_abort.assert_called_once_with(404, message="No such permission with ID=1.0")
    ...
    @patch('department_app.rest.permission.abort')
    def test_post_permission_not_valid(self, mok_abort):
        """Api should return information about permission."""
        with app.test_request_context(json=self.post_invalid_data):
            PermissionList.post()
    ...
    @patch('department_app.rest.permission.abort')
    def test_post_permission_wrong_json(self, mock_abort):
        """Api should return information about permission."""
        with app.test_request_context(json=self.wrong_json):
            PermissionList.post()
